...
//force to update FW, add by shenzhiwang, @20201016 {
  memset(valueStr, 0, sizeof(valueStr));
  len = property_get("persist.nfc.firstboot", valueStr, "true");
  NXPLOG_NCIHAL_D("check FW,len=%d,valueStr=%s", len, valueStr);
  if (len > 0 && !strcmp(valueStr, "true")) {
    NXPLOG_NCIHAL_D("force to update FW");

    *fw_update_req = true;
    property_set("persist.nfc.firstboot", "false");
  }
//@}
